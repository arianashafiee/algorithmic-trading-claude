#!/usr/bin/env node
// src/pipeline/examples/startDataPipeline.js
 ...
 */
async function main() {
  const logger = new PipelineLogger('PipelineExample');
  
  try {
    logger.info('Starting CC Trading Terminal Data DataIngestionPipeline...');
    ...
    setInterval(async () => {
      try {
        ...
        logger.info('DataIngestionPipeline Health Check', {
    ...
    process.on('SIGINT', async () => {
      ...
      try {
        ...
        logger.info('DataIngestionPipeline shutdown completed');
        process.exit(0);
      } catch (error) {
    ...
    logger.info('DataIngestionPipeline is running. Press Ctrl+C to stop.');
