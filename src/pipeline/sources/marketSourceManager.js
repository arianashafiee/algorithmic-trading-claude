// src/pipeline/sources/marketSourceManager.js
import EventEmitter from 'events';
import { PipelineLogger } from '../monitoring/pipelineLogger.js';
import { CryptoMarketSource } from './cryptoMarketSource.js';
import { TradingTerminalSource } from './tradingTerminalSource.js';
...
import { normalizeSourceConfig, validateConfiguredSource } from '../utils/sourceConfig.js';
 ...
 */
export class MarketSourceManager extends EventEmitter {
  constructor(options = {}) {
    super();
    
    this.logger = new PipelineLogger('MarketSourceManager');
    this.options = {
  ...
  initializeAvailableSources() {
    ...
    this.availableSources.set('cc-trading-terminal', TradingTerminalSource);
    ...
    this.availableSources.set('crypto-generic', CryptoMarketSource);
  ...
  async addSource(sourceConfig) {
    ...
    const { id, type, enabled = true } = normalizedConfig;
    
    try {
      if (!validateConfiguredSource(normalizedConfig)) {
        throw new Error('Data source configuration requires id and type');
      }
      ...
      this.sourceConfigs.set(id, normalizedConfig);
      this.metrics.messagesBySource[id] = this.metrics.messagesBySource[id] || 0;
      this.metrics.errorsBySource[id] = this.metrics.errorsBySource[id] || 0;
      ...
      this.logger.error(`Failed to add data source: ${id || 'unknown'}`, error);
      throw error;
    }
  }
  ...
  async startSource(sourceId) {
    try {
      ...
      this.metrics.messagesBySource[sourceId] = this.metrics.messagesBySource[sourceId] || 0;
      this.metrics.errorsBySource[sourceId] = this.metrics.errorsBySource[sourceId] || 0;
  ...
  async stopSource(sourceId) {
    try {
      clearTimer(this.reconnectTimers.get(sourceId));
  ...
  handleSourceError(sourceId, error) {
    this.metrics.errorsBySource[sourceId] = (this.metrics.errorsBySource[sourceId] || 0) + 1;
  ...
  scheduleReconnect(sourceId) {
    const source = this.activeSources.get(sourceId);
    if (!source || this.reconnectTimers.has(sourceId)) return;

    const timer = setTimeout(async () => {
      this.reconnectTimers.delete(sourceId);

      try {
        const attempts = source.reconnectAttempts || 0;
        ...
        if (typeof source.reconnect === 'function') {
  ...
  generateMessageId(sourceId) {
    return createMessageId(sourceId);
  }
  ...
  async initialize() {
    ...
    this.logger.info('MarketSourceManager initialized');
  }
  ...
  async shutdown() {
    try {
      // Clear health check intervals and reconnect timers
      clearTimer(this.healthCheckInterval);
      ...
      this.logger.info('MarketSourceManager shutdown completed');

    } catch (error) {
      this.logger.error('Error during MarketSourceManager shutdown', error);
      throw error;
    }
  }
}

export { MarketSourceManager as DataSourceManager };
