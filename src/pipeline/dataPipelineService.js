// src/pipeline/dataPipelineService.js
import EventEmitter from 'events';
import { PipelineLogger } from './monitoring/pipelineLogger.js';
import { DataIngestionPipeline } from './core/dataIngestionPipeline.js';
import { MarketSourceManager } from './sources/marketSourceManager.js';
import { HistoricalMarketDataService } from './storage/historicalMarketDataService.js';
import { normalizeSourceConfig } from './utils/sourceConfig.js';

/**
 * Main data pipeline service that orchestrates all components
...
export class DataPipelineService extends EventEmitter {
  constructor(options = {}) {
    ...
    this.logger = new PipelineLogger('DataPipelineService');
    this.options = {
    ...
    // DataIngestionPipeline configuration
    this.pipelineConfig = {
  ...
  async initialize() {
    if (this.isInitialized) {
      this.logger.warn('Data pipeline service already initialized');
      return;
    }

    try {
      this.logger.info('Initializing data pipeline service...');
      ...
      this.logger.info('Data pipeline service initialized successfully');
      this.emit('initialized');

    } catch (error) {
      this.logger.error('Failed to initialize data pipeline service', error);
      throw error;
    }
  }
  ...
  async initializePipeline() {
    this.pipeline = new DataIngestionPipeline({
  ...
  async initializeDataSourceManager() {
    this.dataSourceManager = new MarketSourceManager({
      autoReconnect: true,
      reconnectDelay: 5000,
  ...
  async initializeHistoricalDataService() {
    this.historicalDataService = new HistoricalMarketDataService({
  ...
  setupEventHandlers() {
    ...
    this.pipeline.on('error', (error) => {
      this.logger.error('Data ingestion pipeline error', error);
  ...
  async configureDataSources() {
    for (const sourceConfig of this.pipelineConfig.dataSources) {
      try {
        ...
        await this.dataSourceManager.addSource(normalizedConfig);
        this.logger.info(`Configured data source: ${normalizedConfig.id}`);
      } catch (error) {
        this.logger.error(`Failed to configure data source: ${sourceConfig.id || 'unknown'}`, error);
      }
    }
  }
  ...
  async start() {
    ...
    if (this.isRunning) {
      this.logger.warn('Data pipeline service already running');
      return;
    }

    try {
      this.logger.info('Starting data pipeline service...');
      ...
      this.logger.info('Data pipeline service started successfully');
      this.emit('started');

    } catch (error) {
      this.logger.error('Failed to start data pipeline service', error);
      throw error;
    }
  }
  ...
  async stop() {
    if (!this.isRunning) {
      this.logger.warn('Data pipeline service not running');
      return;
    }

    try {
      this.logger.info('Stopping data pipeline service...');
      ...
      this.logger.info('Data pipeline service stopped successfully');
      this.emit('stopped');

    } catch (error) {
      this.logger.error('Error stopping data pipeline service', error);
      throw error;
    }
  }
  ...
  async addDataSource(sourceConfig) {
    ...
    this.pipelineConfig.dataSources.push(normalizedConfig);
  }
  ...
  async shutdown() {
    try {
      ...
      this.logger.info('Data pipeline service shutdown completed');
      this.emit('shutdown');

    } catch (error) {
      this.logger.error('Error during data pipeline service shutdown', error);
      throw error;
    }
  }
}

export { DataPipelineService as PipelineService };
