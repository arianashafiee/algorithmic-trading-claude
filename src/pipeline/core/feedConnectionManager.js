// src/pipeline/core/feedConnectionManager.js
 ...
 */
export class FeedConnectionManager extends EventEmitter {
  constructor(options = {}) {
    super();
    
    this.logger = new PipelineLogger('FeedConnectionManager');
    this.options = {
      maxConnections: 50,
  ...
  async reconnect(id) {
    ...
    // Remove from Map so connect() can add a fresh entry
    ...
    // Reset reconnect attempts on the newly registered connection entry
    const newEntry = this.connections.get(id);
    if (newEntry) {
...
export { FeedConnectionManager as ConnectionManager };
