// src/pipeline/core/apiRateLimiter.js
...
export class ApiRateLimiter extends EventEmitter {
  constructor(options = {}) {
    ...
    this.logger = new PipelineLogger('ApiRateLimiter');
    this.options = {
  ...
  async initialize() {
    if (this.isInitialized) {
      this.logger.warn('API rate limiter already initialized');
      return;
    }
    ...
    this.logger.info('API rate limiter initialized');
  }
  ...
  async shutdown() {
    ...
    this.isInitialized = false;
    this.limiters.clear();
    this.logger.info('API rate limiter shutdown');
  }
}
