...
export class MarketDataProcessor extends EventEmitter {
  constructor(options = {}) {
    ...
    this.logger = new PipelineLogger('MarketDataProcessor');
    this.options = {
      batchSize: 100,
    ...
    this.validator = new MarketDataValidator(this.options.validation || {});
    this.transformer = new MarketDataTransformer(this.options.transformation || {});
    this.enricher = new MarketDataEnricher(this.options.enrichment || {});
  ...
  async initialize() {
    try {
      ...
      this.logger.info('MarketDataProcessor initialized');
    } catch (error) {
      this.logger.error('Failed to initialize MarketDataProcessor', error);
      throw error;
    }
  }
  ...
  async shutdown() {
    try {
      ...
      this.logger.info('MarketDataProcessor shutdown completed');
    } catch (error) {
      this.logger.error('Error during MarketDataProcessor shutdown', error);
      throw error;
    }
  }
}

export { MarketDataProcessor as DataProcessor };
