// src/pipeline/processors/marketDataTransformer.js
 ...
 */
export class MarketDataTransformer extends EventEmitter {
  constructor(options = {}) {
    super();
    
    this.logger = new PipelineLogger('MarketDataTransformer');
    this.options = {
      enableCaching: true,
      maxCacheSize: 1000,
      defaultTimezone: 'UTC',
   ...
   */
  async initialize() {
    this.logger.info('MarketDataTransformer initialized');
  }
    ...
    this.addTransformation('token', {
      ...
      calculateMarketMetrics: (data) => {
        ...
        if (data.price && data.priceChange24h !== undefined) {
          const prevPrice = data.price - data.priceChange24h;
          if (prevPrice !== 0) {
            data.priceChange24hPercent = (data.priceChange24h / prevPrice) * 100;
          }
        }
  ...
  async shutdown() {
    ...
    this.logger.info('MarketDataTransformer shutdown');
  }
}

export { MarketDataTransformer as DataTransformer };
