// src/pipeline/monitoring/pipelineHealthMonitor.js
...
export class PipelineHealthMonitor extends EventEmitter {
  constructor(options = {}) {
    ...
    this.logger = new PipelineLogger('PipelineHealthMonitor');
    this.options = {
  ...
  async initialize() {
    ...
    this.logger.info('PipelineHealthMonitor initialized');
  }
  ...
  async checkComponent(name, config) {
    ...
    const previousIsHealthy = config.isHealthy;
  ...
  checkForAlerts(componentResults) {
    for (const [name, result] of componentResults.entries()) {
      ...
      // New unhealthy component (was healthy before this check)
      if (!result.isHealthy && result.previousIsHealthy) {
      ...
      // Component recovered (was unhealthy before this check)
      if (result.isHealthy && !result.previousIsHealthy) {
      ...
      // Critical threshold reached (wasn't critical before this check)
      if (result.status === 'critical' && result.previousStatus !== 'critical') {
  ...
  async shutdown() {
    ...
    this.logger.info('PipelineHealthMonitor shutdown completed');
  }
}

export { PipelineHealthMonitor as HealthChecker };
