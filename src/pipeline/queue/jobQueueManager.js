// src/pipeline/queue/jobQueueManager.js
import EventEmitter from 'events';
import { PipelineLogger } from '../monitoring/pipelineLogger.js';
import { MemoryJobQueue } from './memoryJobQueue.js';
import { PersistentJobQueue } from './persistentJobQueue.js';
import { clearTimer, withTimeout } from '../utils/asyncUtils.js';
import { createBatchId } from '../utils/idGenerator.js';
...
export class JobQueueManager extends EventEmitter {
  constructor(options = {}) {
    ...
    this.logger = new PipelineLogger('JobQueueManager');
    this.options = {
  ...
  async initialize() {
    if (this.isInitialized) {
      this.logger.warn('JobQueueManager already initialized');
      return;
    }
    ...
    try {
      ...
      this.logger.info('JobQueueManager initialized');
    } catch (error) {
      this.logger.error('Failed to initialize JobQueueManager', error);
      throw error;
    }
  }
  ...
  async createQueue(name, config = {}) {
    try {
      ...
      if (queueConfig.type === 'persistent') {
        queue = new PersistentJobQueue(name, queueConfig);
      } else {
        queue = new MemoryJobQueue(name, queueConfig);
      }
  ...
  async defaultDataProcessor(data, job) {
    try {
      // This is called by the MarketDataProcessor component
      this.emit('process-data', data, job);
      
      const processedEvent = `processed-${job.id}`;
      const errorEvent = `error-${job.id}`;

      const processingResult = new Promise((resolve, reject) => {
        const cleanup = () => {
          this.off(processedEvent, handleProcessed);
        ...
        this.once(processedEvent, handleProcessed);
        this.once(errorEvent, handleError);
      });

      return await withTimeout(
        processingResult,
        this.options.queueTimeout,
        `Data processing timeout for job ${job.id}`
      );
  ...
  startMetricsCollection() {
    clearTimer(this.metricsInterval);
  ...
  async shutdown() {
    try {
      ...
      this.isInitialized = false;
      this.logger.info('JobQueueManager shutdown completed');

    } catch (error) {
      this.logger.error('Error during JobQueueManager shutdown', error);
      throw error;
    }
  }
}

export { JobQueueManager as QueueManager };
