// src/services/orderExecutionService.js
...
export class OrderExecutionService {
  constructor(solanaService, dexService) {
    ...
    this.ready = this.initializeStorage().then(() => this.loadOrders());
  }
  ...
  async checkOrderExecution(order) {
    ...
    try {
      ...
      const currentOutputAmount = parseInt(quote.bestQuote.outAmount || 0);

      console.log(`Checking order ${order.id}: current output ${currentOutputAmount}, expected ${order.expectedOutput}`);

      // Check if target price is met by comparing raw output amounts
      let shouldExecute = false;
      if (order.type === 'buy') {
        // Execute when DEX gives at least as much output as expected at target price
...
export { OrderExecutionService as SolanaLimitOrderService };
