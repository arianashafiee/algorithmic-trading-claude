// src/services/solanaSwapRouterService.js
import { Transaction, VersionedTransaction } from "@solana/web3.js";
import fetch from "node-fetch";

export class SolanaSwapRouterService {
  ...
  deserializeSwapTransaction(serializedTransaction) {
  ...
  signSwapTransaction(transaction) {
  ...
  async executeJupiterSwap(quote, priorityFee = 0.001) {
    try {
      ...
      const transaction = this.deserializeSwapTransaction(swapData.swapTransaction);
      this.signSwapTransaction(transaction);
  ...
  async executeRaydiumSwap(quote, priorityFee = 0.001) {
    try {
      ...
      const transaction = this.deserializeSwapTransaction(swapData.transaction);
      this.signSwapTransaction(transaction);
  ...
  async getBestQuote(inputToken, outputToken, amount, slippageBps = 50) {
    try {
      ...
      const bestQuote = validQuotes.reduce((best, current) => {
        const currentOut = parseInt(current.outAmount || 0);
        const bestOut = parseInt(best.outAmount || 0);
...
export { SolanaSwapRouterService as SolanaDexService };
